...
func main() {
	...
	// === O CRACHÁ DO CLIENTE (Lido uma vez só, fora do loop) ===
	...
	// === A CHAVE PRIVADA (Lida uma vez só, fora do loop) ===
	...
	// =========================================================
	// 🚀 O TESTE DE ESTRESSE: ENVIANDO MÚLTIPLAS TRANSAÇÕES
	// =========================================================
	quantidadeTransacoes := 10 // Altere aqui se quiser testar com 50, 100, etc.
	fmt.Printf("🚀 Preparando rajada de %d transações...\n", quantidadeTransacoes)

	for i := 1; i <= quantidadeTransacoes; i++ {
		// Gera um ID único para cada transação (ex: SKEEN_BFT_TEST_001)
		txID := fmt.Sprintf("SKEEN_BFT_TEST_%03d", i)

		// Nonce único para evitar Replay Attack
		nonce := make([]byte, 24)
		rand.Read(nonce)
		...
		payload := &common.Payload{
			...
			Data: []byte(fmt.Sprintf("Transação de carga número %d para o Skeen BFT!", i)),
		}
		payloadBytes, _ := proto.Marshal(payload)

		// === A CANETA DIGITAL (Assinando cada pacote individualmente) ===
		...
		// Dispara para o Fabric
		err = stream.Send(env)
		if err != nil {
			log.Fatalf("Erro no envio da Tx %d: %v", i, err)
		}

		// Aguarda o ACK do Fabric antes de mandar a próxima
		reply, err := stream.Recv()
		if err != nil {
			log.Fatalf("Erro na resposta da Tx %d: %v", i, err)
		}

		fmt.Printf("✅ [%s] Aceita! Status: %s\n", txID, reply.Status)
	}

	fmt.Println("🎉 Teste de estresse concluído com sucesso!")
}